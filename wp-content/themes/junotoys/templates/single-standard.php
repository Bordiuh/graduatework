...
if ( !function_exists( 'junotoys_template_single_standard_theme_setup' ) ) {
	add_action( 'junotoys_action_before_init_theme', 'junotoys_template_single_standard_theme_setup', 1 );
	function junotoys_template_single_standard_theme_setup() {
		junotoys_add_template(array(
			'layout' => 'single-standard',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'junotoys'),
			'thumb_title'  => esc_html__('Fullwidth image (crop)', 'junotoys'),
			'w'		 => 1170,
			'h'		 => 659
		));
	}
...
// Template output
if ( !function_exists( 'junotoys_template_single_standard_output' ) ) {
	function junotoys_template_single_standard_output($post_options, $post_data) {
		...
		$title_tag = junotoys_get_custom_option('show_page_title')=='yes' ? 'h3' : 'h1';
		...
		if ($show_title && $post_options['location'] == 'center' && junotoys_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php junotoys_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
		<?php 
		}

		if (!$post_data['post_protected'] && (
			!empty($post_options['dedicated']) ||
			(junotoys_get_custom_option('show_featured_image')=='yes' && $post_data['post_thumb'])	// && $post_data['post_format']!='gallery' && $post_data['post_format']!='image')
		)) {
			?>
			...
			if (!empty($post_options['dedicated'])) {
				...
				<div class="post_thumb" data-image="<?php echo esc_url($post_data['post_attachment']); ?>" data-title="<?php echo esc_attr($post_data['post_title']); ?>">
					<a class="hover_icon hover_icon_view" href="<?php echo esc_url($post_data['post_attachment']); ?>" title="<?php echo esc_attr($post_data['post_title']); ?>"><?php junotoys_show_layout($post_data['post_thumb']); ?></a>
				</div>
				<?php 
			}
			?>
		...
		if ($post_data['post_protected']) { 
			...
			if ( junotoys_get_custom_option('show_post_tags') == 'yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				...
				<div class="post_info post_info_bottom">
					<span class="post_info_item post_info_tags"><?php esc_html_e('Tags:', 'junotoys'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php 
			}
		} 
		...
		if (!$sidebar_present) junotoys_close_wrapper();	// .post_item
