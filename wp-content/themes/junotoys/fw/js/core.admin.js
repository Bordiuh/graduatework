/**
 * Juno Toys Framework: Admin scripts
 *
 * @package	junotoys
 * @since	junotoys 1.0
 */
...
jQuery(document).ready(function() {
	...
	jQuery('.widgets_param_post_type_selector').on('change', function() {
		...
		junotoys_admin_fill_categories(this, cat_fld, cat_lbl);
		return false;
	});
	// Set parent course
	jQuery('.junotoys_course_selector').on('click', function(e) {
		"use strict";
...
function junotoys_admin_fill_categories(fld, cat_fld, cat_lbl) {
	"use strict";
	var cat_value = junotoys_get_listbox_selected_value(cat_fld.get(0));
	cat_lbl.append('<span class="sc_refresh iconadmin-spin3 animate-spin"></span>');
	var pt = jQuery(fld).val();
	// Prepare data
	var data = {
		action: 'junotoys_admin_change_post_type',
