...
( function( $ ) {
	...
	if (!junotoys_previewer_vars['need_refresh']) {
		var $style = $('#junotoys-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="junotoys-customizer-css"></'+'style'+'>' ).find('#junotoys-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			junotoys_set_cookie(junotoys_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
