...
function junotoys_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		junotoys_custom_options_show_loader();
		//location.reload();
		var loc = jQuery('#co_site_url').val();
		var params = JUNOTOYS_STORAGE['co_add_params']!=undefined ? JUNOTOYS_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#junotoys-customizer-styles-'+option).length > 0 ? jQuery('#junotoys-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="junotoys-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
