...
// Theme init
if (!function_exists('junotoys_widget_recent_reviews_theme_setup')) {
	add_action( 'junotoys_action_before_init_theme', 'junotoys_widget_recent_reviews_theme_setup', 1 );
	function junotoys_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('junotoys_exists_visual_composer') && junotoys_exists_visual_composer())
			add_action('junotoys_action_shortcodes_list_vc','junotoys_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('junotoys_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'junotoys_widget_recent_reviews_load' );
	function junotoys_widget_recent_reviews_load() {
		register_widget('junotoys_widget_recent_reviews');
	}
}

// Widget Class
class junotoys_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'junotoys'));
		parent::__construct( 'junotoys_widget_recent_reviews', esc_html__('ThemeREX - Recent reviews', 'junotoys'), $widget_ops );
...
if ( !function_exists( 'junotoys_sc_widget_recent_reviews' ) ) {
	function junotoys_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'junotoys_widget_recent_reviews';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, junotoys_prepare_widgets_args(junotoys_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('junotoys_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	junotoys_require_shortcode("trx_widget_recent_reviews", "junotoys_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('junotoys_widget_recent_reviews_reg_shortcodes_vc')) {
	function junotoys_widget_recent_reviews_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Reviews", 'junotoys'),
				"description" => wp_kses_data( __("Insert recent reviews list with thumbs and post's meta", 'junotoys') ),
