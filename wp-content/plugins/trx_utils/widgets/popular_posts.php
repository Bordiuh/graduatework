...
// Theme init
if (!function_exists('junotoys_widget_popular_posts_theme_setup')) {
	add_action( 'junotoys_action_before_init_theme', 'junotoys_widget_popular_posts_theme_setup', 1 );
	function junotoys_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('junotoys_exists_visual_composer') && junotoys_exists_visual_composer())
			add_action('junotoys_action_shortcodes_list_vc','junotoys_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('junotoys_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'junotoys_widget_popular_posts_load' );
	function junotoys_widget_popular_posts_load() {
		register_widget('junotoys_widget_popular_posts');
	}
}

// Widget Class
class junotoys_widget_popular_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => esc_html__('The most popular and most commented blog posts (extended)', 'junotoys'));
		parent::__construct( 'junotoys_widget_popular_posts', esc_html__('ThemeREX - Most Popular & Commented Posts', 'junotoys'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? junotoys_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = junotoys_storage_get('options_prefix').'_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = junotoys_storage_get('options_prefix').'_post_likes_count';
				$args['orderby'] = 'meta_value_num';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php esc_html_e('Most popular tab title:', 'junotoys'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php esc_html_e('Most commented tab title:', 'junotoys'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php esc_html_e('Most liked tab title:', 'junotoys'); ?></label>
...
if ( !function_exists( 'junotoys_sc_widget_popular_posts' ) ) {
	function junotoys_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'junotoys_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (junotoys_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, junotoys_prepare_widgets_args(junotoys_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('junotoys_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	junotoys_require_shortcode("trx_widget_popular_posts", "junotoys_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('junotoys_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('junotoys_action_shortcodes_list_vc','junotoys_widget_popular_posts_reg_shortcodes_vc');
	function junotoys_widget_popular_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Popular Posts", 'junotoys'),
				"description" => wp_kses_data( __("Insert popular posts list with thumbs and post's meta", 'junotoys') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Most popular tab title", 'junotoys'),
						"description" => wp_kses_data( __("Most popular tab title", 'junotoys') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most commented tab title", 'junotoys'),
						"description" => wp_kses_data( __("Most commented tab title", 'junotoys') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most liked tab title", 'junotoys'),
						"description" => wp_kses_data( __("Most liked tab title", 'junotoys') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
